...
const RoomDetails = () => {
    const { id } = useParams()
    const { axios, Token, navigate, rooms } = useAppContext()
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            const token = await Token // remove await if Token is a string
            const rid = id
            ...
            console.log({ id, checkInDate, checkOutDate, guests })
    ...
    useEffect(() => {
        ...
        if (foundRoom) {
            setRoom(foundRoom)
            setMainImages(foundRoom.images?.[0] || null)
        }
    }, [rooms, id])
    ...
    return room && (
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            <div className='flex flex-col md:flex-row items-start md:items-center gap-2'>
                <h1 className='text-3xl md:text-4xl font-playfair'>{room.hotel.name}<span className='font-inter text-sm px-2'>({room.roomType})</span></h1>
                <p className='text-xs font-inter py-1.5 px-3 text-white bg-orange-500 rounded-full'>20% OFF</p>
            </div>
            {/* {room rating} */}
            <div className='flex items-center gap-1 mt-2'>
                <StarRating />
                <p className='px-6'>200+ Reviews</p>
            </div>
            {/* {room-address} */}
            ...
            <div className='flex flex-col lg:flex-row mt-6 gap-6'>
                <div className='lg:w-1/2 w-full'>
                    {mainImages && (
                        ...
                        <img src={mainImages} alt="room image" className='w-full rounded-xl shadow-lg object-cover' />
                    )}
                </div>
                ...
                <div className='grid grid-cols-2 gap-4 lg:w-1/2  w-full'>
                    {room?.images.length > 1 && room.images.map((image, index) => (
                        <img onClick={() => setMainImages(image)}
                            key={index} src={image} alt='room-images'
                            className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImages === image && 'outline-3 outline-orange-500'}`}></img>
                    ))}
                </div>
            </div>
            {/* {room highlights} */}
                ...
                <div className=' flex flex-col justify-between flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500 '>
                    ...
                    <div className='flex flex-col'>
                        ...
                        <input onChange={(e) => setGuests(e.target.value)} value={guests}
                            type="number" id='Guest' placeholder='1' className='max-w-20 rounded  border border-gray-300 px-2 py-2 mt-1.5 outline-none' required />
                    </div>
                </div>
            ...
            <div className='flex flex-col items-start gap-4'>
                <div className='flex gap-4'>
                    <img src={room.hotel?.owner?.images}
                ...
                <button className='bg-primary hover:bg-primary-dull active:scale-95 transition-all text-white rounded cursor-pointer px-6 py-2.5'>Contact Now</button>
            </div>
        </div>
    )
}
