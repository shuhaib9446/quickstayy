...
const HotelReg = () => {
 ...
 const onSubmitHandler=async(event)=>{
    try {
        ...
        const {data}=await axios.post(`/api/hotels/`,{name,contact,address,city},{headers:{Authorization:`Bearer ${await Token}`}})
  ...
  return (
    <div onClick={()=>setShowHotelReg(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/70'>
        <form onSubmit={onSubmitHandler} onClick={(e=>e.stopPropagation())} className='flex bg-white rounded-xl max-w-4xl max-md:2'>
            <img src={assets.regImage} alt="reg-hotel-image" className='w-1/2 rounded-xl hidden md:block'/>
            ...
            <div className='w-full mt-4 max-w-60 mr-auto'>
                ...
                <select id='city' onChange={(e)=>setCity(e.target.value)} value={city} className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light'required>
                    ...
                    {cities.map((city)=>(
                        <option key={city} id={city} value={city}>{city}</option>
                    ))}
                </select>
            </div>
